...
@pytest.mark.parametrize("changed", [False, True])
def test_first_group_retry_checks_source_bytes(changed):
    driver, trainer, model = make_driver()
    driver.arm_next_member(trainer, model)
    prepared = trainer._psm_active_armed_prepared
    registry = driver.registry
    plan = registry.abort_source_transient(prepared)
    segment = prepared.segment
    if changed:
        segment.evidence_visual_summary_prev[0, 1, 0] += 1
        with pytest.raises(RuntimeError, match="RETRY_SOURCE_CHANGED"):
            registry.prepare_retry(segment, plan, trainer_grad_accum_iter=0)
        assert registry.owner.phase is RuntimePhase.ABORTED
    else:
        retried = registry.prepare_retry(segment, plan, trainer_grad_accum_iter=0)
        assert retried.identity is prepared.identity
        assert retried.inputs.identities == prepared.inputs.identities
    assert not registry.owner.scheduler.committed_identities
    assert not registry.owner.adapter.sidecar._records


def test_source_failure_blocks_checkpoint_and_clears_all_slow_grads(monkeypatch):
    driver, trainer, model = make_driver()
    owner = driver.registry.owner
    before = owner.snapshot()
    for parameter in owner.wiring.local_slow_parameters:
        parameter.grad = torch.ones_like(parameter)
    original = driver.producer.produce

    def fail_later(stream, *, cursor):
        if stream.slot_id == 1:
            raise OSError("injected source failure")
        return original(stream, cursor=cursor)

    monkeypatch.setattr(driver.producer, "produce", fail_later)
    with pytest.raises(OSError, match="injected"):
        driver.arm_next_member(trainer, model)
    assert owner.phase is RuntimePhase.ABORTED
    assert owner.scheduler.snapshot() == before.scheduler
    assert not owner.adapter.sidecar._records
    assert all(p.grad is None for p in owner.wiring.local_slow_parameters)
...
def test_cuda_vectorized_fast_state_and_outer_gradients_match_scalar():
    ...
    try:
        ...
        expected, state = _original_row_update(reference, evidence, reference.initial_state(8, device="cuda"), valid)
...
def test_delivery_metrics_dynamic_callback_retains_trainer_and_exact_counts(tmp_path):
    import json
    ...
    callback = ActiveDeliveryMetrics(trainer=trainer, driver=driver, path=tmp_path / "metrics.jsonl")
    assert callback.trainer is trainer
    run_window(driver, trainer, model)
    model._psm_native_forward_calls = trainer.config.trainer.grad_accum_iter
    callback._losses = [1.0] * trainer.config.trainer.grad_accum_iter
    callback._waves = [1] * trainer.config.trainer.grad_accum_iter
    callback._identities = [(0, "test", i) for i in range(driver._group_plan.n_window)]
    callback.on_training_step_end(model, {}, {}, torch.tensor(1.0), iteration=1)
    record = json.loads((tmp_path / "metrics.jsonl").read_text())
    assert record["native_forwards"] == 2
    assert record["valid_consumers"] == 8
    assert record["window_index"] == 1
    assert callback._losses == []
