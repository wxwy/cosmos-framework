        ...
        if cached_latents is not None and (not isinstance(cache_enabled, torch.Tensor) or bool(cache_enabled.all())):
            ...
            if isinstance(cached_latents, (list, tuple)) and len(cached_latents) == len(raw_state_vision):
                ...
                for latent in cached_latents:
                    ...
                    # Keep fp32 (online _encode_vision_item returns fp32); only move
                    # device so the cached x0 matches the online noising path dtype.
                    x0_tokens_vision.append(
                        latent.permute(1, 0, 2, 3).unsqueeze(0).to(
                            device=self.tensor_kwargs_fp32["device"], dtype=torch.float32
                        )
                    )
