...
class ActionTransformPipeline:
    ...
    def __call__(
        ...
        if window_history_latents is not None:
            ...
            # Keep every WINDOW sample on the same multi-vision ABI, including
            # episode start where no completed history exists yet. Mixed batches are
            # common: if a start sample remains a bare tensor while another sample has
            # multiple vision items, the model's batch-level multi-vision probe would
            # read the bare video's channel dimension as its item count.
            ...
            # action_start_frame_offset is 1 by default; keep it explicit for the
            # native-window route. For H>0 this restores aligned history action
            # semantics; for H=0 it is identical to the ordinary WAM plan.
