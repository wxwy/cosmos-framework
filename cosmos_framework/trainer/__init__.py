...
def _canonical_native_cpu_static_topology_error(trainer: Any, model_ddp: torch.nn.Module) -> str | None:
...
class ImaginaireTrainer:
    ...
    def training_step(
        ...
        if active_data.get("canonical_production_segment_mode") is True:
            if isinstance(optimizer, torch.optim.Optimizer):
                raise RuntimeError("canonical native CPU/static route rejects scaler or optimizer before scan: optimizer")
            if grad_scaler.is_enabled():
                raise RuntimeError("canonical native CPU/static route rejects scaler or optimizer before scan: enabled scaler")
            topology_error = _canonical_native_cpu_static_topology_error(self, model_ddp)
            if topology_error is not None:
                raise RuntimeError(f"canonical native CPU/static route rejects {topology_error} before scan")
