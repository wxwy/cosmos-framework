...
# 在 tmux 会话中启动 libero4in1 正式 SFT（Edge base + 4 suite，max 2000 step）。
#
# 背景：容器 zshrc 把 `tmux` 别名成坏插件函数 _zsh_tmux_plugin_run，直接敲 `tmux`
# 会报 command not found；这里固定用 /usr/bin/tmux 全路径绕过。
#
# 用法：
#   bash examples/tmux_launch_sft_libero_edge_all.sh [tmux_session_name]
#   默认 session=sft_4in1
set -euo pipefail
...
/usr/bin/tmux new-session -d -s "$SESSION" -x 200 -y 50 \
  "cd $PWD && export PATH=$PWD/.venv/bin:\$PATH && export LIBERO_ROOT='$LIBERO_ROOT' && export BASE_CHECKPOINT_PATH='$BASE_CHECKPOINT_PATH' && export EDGE_POLICY_CHECKPOINT='$EDGE_POLICY_CHECKPOINT' && export LOG_FILENAME=action_policy_libero_edge_all_sft.log && bash examples/launch_sft_action_policy_libero_edge_all.sh"
