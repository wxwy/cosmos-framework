...
# Structured-TOML launch for action_policy_libero_edge_warmstart — Edge-Policy-DROID
# warm-start LIBERO-10 action-policy SFT (单卡 A100-80GB 版). Drives
# cosmos_framework.scripts.train against
# examples/toml/sft_config/action_policy_libero_edge_warmstart.toml.
#
# 官方 nano 超参全保留（bs=128/rank、74000 tokens、lr 5e-5、cycle 16000/500、selective AC）；
...
#     bash examples/launch_sft_action_policy_libero_edge_warmstart.sh
#   # smoke:
#   LIBERO_ROOT=/disk/data/LIBERO_LeRobot_v3/libero_10 \
#     EXTRA_TAIL_OVERRIDES="trainer.max_iter=5 trainer.logging_iter=1" \
#     bash examples/launch_sft_action_policy_libero_edge_warmstart.sh

TOML_FILE="examples/toml/sft_config/action_policy_libero_edge_warmstart.toml"
